...
rc$Tool[rc$Tool == "Kraken-II (concatenated)"] <- "Kraken-II"
rc$Tool[rc$Tool == "CLARK (concatenated)"] <- "CLARK"
...
ggplot(melt(rc[, 0:4], id.vars = 1:2)) +
  ...
  scale_y_continuous(trans = "log2", name = "Memory (GB)", c(4, 16, 32, 64, 128), sec.axis = sec_axis(~., name="Run-time (minutes)", breaks = c(1, 5, 15, 60, 120))) +
...
ggsave2("../figures/resource_tool_comparison.pdf", width = 6.75, height = 4)