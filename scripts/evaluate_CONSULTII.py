...
def evaluate_classification(result_file):
    ...
    for read_line in all_reads:
        ...
        if pred_vote > TVTH:
            ...
            evaluation_dict["vote"].append("0")
            evaluation_dict["voteNormalized"].append("NA")
...
if __name__ == "__main__":
    ...
    consult_result_dir = Path(RESULTS_DIR)
    # Note that we have CONSULTII output files with filenames: "output_1000x_<GENOME_NAME>.txt".
    ...
    all_evaluations = [
        pool.map(
            ...
            [
                file
                for file in list(consult_result_dir.iterdir())
