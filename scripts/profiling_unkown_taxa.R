...
pm <- pm %>% filter(tool %in% c("CONSULT-II (Eq. 6 without unclassified)", "CONSULT-II (Eq. 6)"))
pm$tool[pm$tool == "CONSULT-II (Eq. 6 without unclassified)"] <- "without unclassified taxon"
pm$tool[pm$tool == "CONSULT-II (Eq. 6)"] <- "with unclassified taxon"
...
ut <- read_tsv("../results/unclassified_taxon.txt")
ut$rank <- factor(
  ut$rank,
...
ut <- ut %>%
  group_by(rank) %>%
  summarise(lower=min(portion), upper=max(portion), p=mean(portion))
...
plot_grid(figperf, plot_grid(ggplot() + theme_void(), figut, ggplot() + theme_void(), ncol=1, rel_heights = c(1,2,1)), rel_widths = c(4, 2.5), ncol=2, labels=c("a", "b"), label_fontfamily = "SansSerif")
