...
ggplot() +
  ...
  geom_function(fun = function(x, s) 1.5 / log2(2**((x - 1) / s) + 2), args = list(s = 6), linewidth = 1, aes(color = "new soft-LCA function")) +
...
ggplot(data.frame(x = c(1, 16)), aes(x)) +
  ...
  mapply(FUN = function(L, K) {
    stat_function(size=1.5, fun = function(x, k, l, SL) (1 + 150 - SL) * f(x, k, l, SL), args = list(k = K, l = L, SL = SL), aes(color = as.factor(K), linetype = as.factor(L)))
  }, L = rep(c(2, 4, 1), each = 5), K = c(6:10) * 2) +
  ...
  theme_minimal_grid(font_size = 16) +  theme(legend.key.width = unit(2,"cm"))
ggsave2("../figures/expected_num_matches.pdf", width = 8, height = 6)
